...
export class VehicleCatalogController {
  ...
  @ApiOperation({
    summary: 'Crear modelo de catálogo de vehículo',
    description:
      'Crea un nuevo modelo de vehículo en el catálogo (no está ligado a una sucursal).',
  })
  @ApiResponse({
    status: 201,
  ...
  @ApiOperation({
    summary: 'Listar catálogo de vehículos',
    description:
      'Devuelve los modelos de vehículo disponibles en el catálogo. Por defecto omite los archivados (isActive=false); use includeInactive=true para incluirlos.',
  })
  ...
  @ApiOperation({
    summary: 'Obtener modelo de catálogo de vehículo',
  ...
  @ApiOperation({
    summary: 'Actualizar modelo de catálogo de vehículo',
  ...
  update(
    @Param('id') id: string,
    @Body() updateVehicleCatalogDto: UpdateVehicleCatalogDto,
  ) {
    return this.vehicleCatalogService.update(id, updateVehicleCatalogDto);
  }

  @ApiOperation({
    summary: 'Eliminar modelo de catálogo de vehículo',
    description:
      'Elimina un modelo de catálogo. Falla si existen vehículos dados de alta con ese modelo; en ese caso, archívelo (PATCH con isActive: false) en vez de eliminarlo.',
  })
  @ApiParam({
    name: 'id',
    description: 'Identificador del modelo de catálogo.',
